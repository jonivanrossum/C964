...
def user_input_features():
    acousticness = st.sidebar.slider('Acousticness', 0.0, 1.0, 0.62)
    danceability = st.sidebar.slider('Danceability', 0.0, 1.0, 0.63)
    duration_ms = st.sidebar.slider('Duration_ms', 5000, 533800, 200773)
    energy = st.sidebar.slider('Energy', 0.0, 1.0, 0.4)
    ...
    liveness = st.sidebar.slider('Liveness', 0.0, 1.0, 0.09)
    loudness = st.sidebar.slider('Loudness', -54.0, 3.2, -12.57)
    speechiness = st.sidebar.slider('Speechiness', 0.0, 1.0, 0.54)
    tempo = st.sidebar.slider('Tempo', 0.0, 220.0, 57.55)
    time_signature = st.sidebar.slider('Time_Signature', 0, 5, 4)
    valence = st.sidebar.slider('Valence', 0.0, 1.0, 0.38)
